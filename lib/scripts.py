...
from mhudtools import *

class SaveFileName():
    def __init__(self, sample_id, str_rad, str_diel):
        folder = './dat/{0}/{0}_{1}/'.format(sample_id, str_rad)
        ...
        str_append = '{0}_{1}_{2}'.format(sample_id, str_rad, str_diel)
...
def CreateFileRun(files, numbands):
    ...
    new0 = str(numbands)
    new1 = '{0}\/{0}_{1}'.format(files.sample_id, files.rad)
    ...
    new_TE = [new0, new1, new2_TE, new3_TE]
...
def Create(sample_id, n_s, rad, dielBack, dielCyl, N, n_c, coords, N_part, numbands):
    ...
    files = SaveFileName(sample_id, str_rad, str_diel)
    ...
    curr_run_TM = './{0}_{1}/{2}'.format(sample_id,str_rad,files.NAME_TM_RUN)
    curr_run_TE = './{0}_{1}/{2}'.format(sample_id,str_rad,files.NAME_TE_RUN)
